...
#take a quick look at the VCDB
summary(vcdb)
head (vcdb)
tail(vcdb) 

#requires package: verisr
#requires object:VCDB

actors <-getnum(vcdb, "actor")
print(actors)

#requires package: verisr
#requires object:VCDB
actors <-getnum(vcdb, "actor", add.n=TRUE, add.freq=TRUE)
print(actors)


#sample image object from veris courtesy of Jay Jacobs
...
# library(ggplot2)
# verisplot<-function(vcdb, field){
#   localdf <-getenum(vcdb, field, add.freq=T) #get the data frame for frequency
#   localdf <-localdf[c(1:5), ] #take first 5 fields of dataframe
#   localdf$lab <- paste(round(localdf$freq*100,0), "%", sep="") #add a label
#   #now start the graphing in ggplot2
#   gg <- ggplot(localdf, aes(x=enum, y=freq, label=lab))
#   gg <- gg + geom_bar(stat="identity", fill ="steelblue")
#   gg <-gg + geom_text(hjust=-0.1, size=3)
#   gg <- gg+coord_flip() + ggtitle(field)
...
#   
# }
# localdf <-localdf[c(1:5), ] #get the data frame
